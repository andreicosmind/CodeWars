...
For example, if given the input 'stress', the function should return 't', since the letter t only occurs once in the string, and occurs 
first in the string.
...
def first_non_repeating_letter(string):
    a = [x for x in string if string.lower().count(x.lower()) == 1]
    if a == []: return ""
    else: return a[0]
